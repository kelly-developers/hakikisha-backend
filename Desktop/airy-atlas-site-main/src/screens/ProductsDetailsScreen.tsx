...
const ProductsDetailsScreen: React.FC<ProductDetailsProps> = ({route}) => {
  ...
  const navigation =
    useNavigation<StackNavigationProp<RouteTabsParamList, 'Home'>>();

  const GoBack = () => {
    navigation.goBack();
  };
  const NavigateToCart = () => {
    // Navigation removed - no cart in HAKIKISHA
    navigation.goBack();
  };
  return (
        ...
        <View className="mb-5">
          ...
          <View className="flex my-5 flex-row mx-5 justify-between ">
            ...
            <View className="flex flex-row gap-x-3 ">
              {FeaturesData.map((item: {id: number; title: string; image: any}) => (
                <View
